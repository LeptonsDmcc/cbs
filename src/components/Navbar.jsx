  ...
  return (
    <header className="top-0 left-0 right-0 h-[80px] bg-white shadow-lg z-20 fixed">
      <nav className="container mx-auto px-4 py-4 flex justify-between items-center">
        <Link to="/"><img className="w-[65px]" src={carPro} alt="CarPro Logo" /></Link>
        {!isAuthenticated && (
          <Link to="/" className="flex items-center ml-8 px-4 py-2 text-gray-800 hover:bg-gray-100 font-bold">
            Log In
          </Link>
        )}
        <div className="flex items-center space-x-6">
            ...
            {isAvatarDropdownOpen && (
              <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg py-2 z-50">
                ...
                <button 
                  ...
                  className="flex items-center px-4 py-2 mt-6 text-gray-800 hover:bg-gray-100 w-full text-left"
                >
                  <CiLogout className="mr-2" /> Log Out
                </button>
              </div>
